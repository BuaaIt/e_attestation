...
const getAllAvenant = async (req, res, next) => {
    ...
    res.status(201).send(avenants.rows);
}
...
const getOneAvenant = async (req, res, next) => {
    ...
    res.status(201).send(avenant.rows[0]);
}
const createAvenant = async (req, res, next) => {
    const {type,created_by,creation_date, description } = req.body;
    try {    
        await pool.query("INSERT INTO avenant (id,type,created_by,creation_date, description) VALUES ($1,$2,$3,$4,$5)", [id,type,created_by,creation_date, description]);
        res.status(200).json({
            "status": "200",
